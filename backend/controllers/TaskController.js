...
class TaskController {
    ...
    static async updateTask(req, res) {
        const tasks = await Tasks.updateTaskByUserId(req.userData.userId, req.body.milestoneId, req.body.title,
            req.body.startDate, req.body.endDate, req.body.progress, req.body.hrsCompleted, req.body.hrsRequired);
        res.status(tasks.code).send({ message: tasks.message, data: tasks.data });
    }

    static async deleteTask(req, res) {
        const tasks = await Tasks.deleteTaskByUserId(req.userData.userId, req.body.taskId);
        res.status(tasks.code).send({ message: tasks.message, data: tasks.data });
    }
