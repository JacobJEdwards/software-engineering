    ...
    static async upcomingTasks() {
        try {
            ...
            for (const user of users) {
                let tasks = await Tasks.upcomingDeadline(user._id.valueOf(), String(Date.now() + 5 * 24 * 60 * 60 * 1000));
                if (tasks.code === 200) {
                    const taskList = tasks.data.map(task => `<li>${task.name} - Due: ${task.deadline}</li>`).join('');
                    const emailData = {
                        from: "Excited User <mail@wonderfultasks.me>",
                        to: [user.email],
