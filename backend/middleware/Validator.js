...
const fullSchema = Joi.object({
  ...
  SemesterStartDate: Joi.date().iso().required(),
  SemesterEndDate: Joi.date().iso().min(Date.now()).required(),
  //
  // Module Validation
  ModuleName: Joi.string().min(3).max(50).required(),
  ModuleCode: Joi.string().alphanum().min(3).max(6).required(),
  ModuleStartDate: Joi.date().iso().required(),
  ModuleEndDate: Joi.date().iso().min(Date.now()).required(),
  ...
  TaskEndDate: Joi.date().iso().min(Date.now()).required(),
  Progress: Joi.string()
    .valid("Started", "In Progress", "Completed")
    .required(),
  hrsCompleted: Joi.number().required(),
  hrsRequired: Joi.number().required().min(Joi.ref("hrsCompleted")),
  activities: Joi.array().items(Joi.string()).required(),

  //Activity Validation
  ActivityUserId: Joi.string().required().alphanum().min(3).max(50),
  TasksList: Joi.array().items(Joi.string()).required(),
