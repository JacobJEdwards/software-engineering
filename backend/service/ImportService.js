...
class ImportService {
  ...
  static async addFileDataUser(file, user) {
    ...
    for (let element of file) {
      let response = await Semester.createSemester(
        element.SemesterName,
        element.SemesterStartDate,
        element.SemesterEndDate,
        user,
      );
      if (response.code !== 200) {
    ...
    for (let element of file) {
      let response = await Module.createModule(
        element.SemesterName,
        element.ModuleName,
        element.ModuleCode,
        element.ModuleStartDate,
        element.ModuleEndDate,
        user,
      );
      if (response.code !== 200) {
    ...
    for (let element of file) {
      let response = await Milestone.createMilestoneByUser(
        user,
        element.ModuleCode,
        element.MilestoneTitle,
        element.MilestoneType,
        element.MilestoneStartDate,
        element.MilestoneEndDate,
        true,
      );
      if (response.code !== 200) {
  ...
  static async updateFileData(file, userid) {
    ...
    for (let element of file) {
      Milestone.deleteMilestoneByUser(
        user,
        element.ModuleCode,
        element.MilestoneTitle,
      );
