...
class ActivityService {
    ...
    static async updateActivity(userId, activityId, newActivityName, newActivityType, newNotes, newHours) {
    ...
    static async readAllMilestoneActivities(userId, moduleCode, milestoneId) {
        ...
        const user = await User.getUserInternal(userId);
        if (user) {
            const milestone = user.semester.find((semester) => semester.modules
    ...
    static async deleteActivity(userid, activityId) {
        ...
        const promises = tasks.map(async (task) => await Task.deleteActivityFromTask(userid, task.valueOf(), activityId, activity.hrsCompleted,),);
