...
class MilestoneService {
  static async createMilestoneByUserId(
    userid,
    moduleCode,
    milestoneName,
    milestoneType,
    ...
    let milestoneExists = user.semester.find((semester) =>
      semester.modules.find((module) => module.moduleCode === moduleCode),
    );
    if (milestoneExists) {
      const newMilestone = {
      ...
      let response = await Validator.validateMilestone(
        milestoneName,
        milestoneType,
        milestoneStartDate,
        milestoneEndDate,
  ...
  static async createMilestoneByUser(
    user,
    moduleCode,
    milestoneName,
    milestoneType,
  ...
  static readMilestoneByUser(user, milestoneId) {
    const milestone = user.semester.flatMap((semester) =>
      semester.modules.flatMap((module) =>
        module.milestones.find((milestone) => milestone.id === milestoneId),
      ),
    );
    if (milestone) {
      return new Response("Milestone found", 200, milestone);
  ...
  static async readMilestoneByUserId(userId, milestoneId) {
    ...
    const milestone = user.module.find((module) =>
      module.milestones.find((milestone) => milestone.id === milestoneId),
    );
    if (milestone) {
      return new Response("Milestone found", 200, milestone);
  ...
  static async updateMilestone(
    ...
    if (milestone && !milestone.ltsDefined) {
      milestone.milestoneName =
        newMilestoneName == null ? milestone.milestoneName : newMilestoneName;
      milestone.milestoneType =
        milestoneType == null ? milestone.milestoneType : milestoneType;
      milestone.startDate =
        newStartDate == null ? milestone.startDate : newStartDate;
  ...
  static async updateMilestoneByUserId(
    userId,
    milestoneId,
    newMilestoneName,
    newStartDate,
  ...
  static async milestoneFromToDate(userId, fromDate, toDate) {
    ...
    let milestones = user.semester.flatMap((sem) =>
      sem.modules.flatMap((mod) =>
        mod.milestones.filter(
          (mil) => mil.endDate >= fromDate && mil.endDate <= toDate,
        ),
      ),
    );
  ...
  static async milestoneFromDate(userId, date) {
    ...
    let milestones = user.semester.flatMap((sem) =>
      sem.modules.flatMap((mod) =>
        mod.milestones.filter((mil) => mil.startDate >= date),
      ),
    );
    return new Response("Tasks found", 200, milestones);
  }
